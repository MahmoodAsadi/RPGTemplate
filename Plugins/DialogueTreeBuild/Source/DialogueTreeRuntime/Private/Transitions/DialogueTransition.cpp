...
void UDialogueTransition::StartTransition()
{
	...
	//Get speaker
	FGameplayTag Speaker = OwningNode->GetSpeaker();
	if (!Speaker.IsValid())
	{
		UE_LOG(
			LogDialogueTree,
	...
	float bIsChoice = OwningNode->GetDetails().bIsChoice;
	// Disable Play time, not needed for now
	...
	// If it is a player's choice, transition out as well.
	if ((bMinPlayTimeElapsed && bAudioFinished) || bIsChoice)
	{
		TransitionOut();
	}
}
