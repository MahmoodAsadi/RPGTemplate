...
FGameplayTag UDialogueSpeechNode::GetSpeaker() const
{
	return Details.SpeakerTag;
}
...
void UDialogueSpeechNode::EnterNode()
{
	...
	if (!Dialogue->SpeakerIsPresent(Details.SpeakerTag))
	{
		UE_LOG(
			LogDialogueTree,
			Error,
...
{
	// FGameplayTag Speaker = GetSpeaker();
	//
	// if (Speaker.IsValid())
	// {
	// 	//Play any audio
