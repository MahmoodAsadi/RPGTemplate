...
bool UDialogueManagerSubsystem::PlayDialogueWithActor(UDialogue* DialogueToPlay, AActor* InActor)
{
	if(!IsValid(DialogueToPlay) || !IsValid(DialogueController))
		return false;

	if (InActor)
		DialogueController->StartDialogueWithActor(DialogueToPlay, InActor);
	else
...
bool UDialogueManagerSubsystem::PlayDialogue(UDialogue* DialogueToPlay, const bool bIsHUDDialogue)
{
	if(!IsValid(DialogueToPlay) || !IsValid(DialogueController))
		return false;

	DialogueController->StartDialogue(DialogueToPlay, bIsHUDDialogue);
	return true;
}
