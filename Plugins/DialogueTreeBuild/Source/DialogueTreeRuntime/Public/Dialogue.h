...
public: 
	...
	* @param InSpeakerTag - FGameplayTag, the gameplay tag 
	* that represent the speaker. 
	*/
	// UFUNCTION(BlueprintCallable, Category = "Dialogue")
	// void SetSpeaker(FName InName, FGameplayTag InSpeakerTag);
	...
	* speaker
	* @return FGameplayTag, the gameplay tag 
	* that represent the speaker.  
	*/
	// UFUNCTION(BlueprintCallable, Category = "Dialogue")
	// FGameplayTag GetSpeaker(FName InName) const;

	/**
	* Adds a speaker entry for the given gameplay tag
	* 
	* @param InTag - FGameplayTag, tag to create an entry for.
	*/
	void AddSpeakerEntry(FGameplayTag InTag);
	...
	* Gameplay tags to associate with expected speaker names. 
	*/
	...
	*/
	...
	/**
	* Retrieves the entire array of expected speaker gameplay tags
	* 
	* @return TArray<FGameplayTag>, the array of 
	* speaker gameplay tags. 
	*/
	UFUNCTION(BlueprintCallable, Category="Dialogue")
	TArray<FGameplayTag> GetAllSpeakers() const;
	...
	*/
	UFUNCTION(BlueprintCallable, Category="Dialogue")
	bool SpeakerIsPresent(const FGameplayTag SpeakerTag) const;
...
public:
	...
	*/
	const TMap<FGameplayTag, FSpeakerField>& GetSpeakerRoles() const;
...
private: 
	...
	*/
	...
	/**
	* Refreshes the speakers, plugging in the provided Gameplay tags. 
	* 
	* @param InSpeakerTags - TArray<FGameplayTag>,
	* the speaker gameplay tags to enter
	*/
	void FillSpeakers(TArray<FGameplayTag> InSpeakerTags);

private:
	/** Editable speaking roles for the graph */
	UPROPERTY(EditAnywhere, NoClear, Category = "Dialogue", 
		meta=(Categories = "NPC", NoResetToDefault))
	TMap<FGameplayTag, FSpeakerField> SpeakerRoles;
	...
	/** A mapping of speaker names to their found gameplaytags */
	UPROPERTY()
