...
FLinearColor UGraphNodeDialogueSpeech::GetNodeTitleColor() const
{
    ...
    FGameplayTag SpeakerTag = Speaker.Speaker->GetSpeakerTag();
    return Graph->GetSpeakerColor(SpeakerTag);
}
...
bool UGraphNodeDialogueSpeech::CanCompileNode()
{
    ...
    if (TransitionType 
        ...
        && Graph)
    {
        if (Graph->HasSpeaker(Speaker.Speaker->GetSpeakerTag()))
        {
            SetErrorFlag(false);
            return true;
        }
...
void UGraphNodeDialogueSpeech::OnPinUpdated()
{
	...
	if (DialogueChildren.Num() > 1)
	{
		SetTransitionType(UInputDialogueTransition::StaticClass());
	}
	else
	{
		SetTransitionType(UAutoDialogueTransition::StaticClass());
	}
}
