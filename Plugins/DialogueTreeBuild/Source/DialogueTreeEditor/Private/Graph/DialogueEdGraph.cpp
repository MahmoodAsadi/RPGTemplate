...
bool UDialogueEdGraph::HasSpeaker(FGameplayTag InTag) const
{
	return GetDialogue()->GetSpeakerRoles().Contains(InTag);
}

FColor UDialogueEdGraph::GetSpeakerColor(FGameplayTag InTag) const
{
	if (GetDialogue()->GetSpeakerRoles().Contains(InTag))
	{
		return GetDialogue()->GetSpeakerRoles().Find(InTag)->GraphColor;
	}
