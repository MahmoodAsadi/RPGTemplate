	...
	FGraphContextMenuBuilder& ContextMenuBuilder) const
{
	...
	GetChoiceNodeMenuActions(ContextMenuBuilder);
}
	...
	FGraphContextMenuBuilder& ContextMenuBuilder) const
{
	...
	{
		if (!Speaker->GetSpeakerTag().IsValid())
		{
			continue;
		}
		...
		TSharedPtr<FNewDialogueNodeAction> NewNodeAction =
			MakeCreateSpeechNodeAction(
				Speaker,
				UAutoDialogueTransition::StaticClass(),
		...
		TSubclassOf<UDialogueTransition> TransitionType, UObject* Outer) const
{
	...
	FText MenuText = FText::Format(
		LOCTEXT("SpeechNodeCreationText", "{0}"),
		FText::FromString(Speaker->GetSpeakerTag().ToString())
	);
	FText MenuTooltip =
...
void UDialogueEdGraphSchema::GetChoiceNodeMenuActions(FGraphContextMenuBuilder& ContextMenuBuilder) const
{
	//Fetch the current graph 
	const UDialogueEdGraph* TargetGraph = 
		Cast<UDialogueEdGraph>(ContextMenuBuilder.CurrentGraph);
	...
	if (!TargetGraph)
		return;
	if (!TargetGraph->GetAllSpeakers().IsValidIndex(0))
		return;
	
	//Category
	FText MenuCategory = LOCTEXT("ChoiceNodeMenuCategory",
		"Choice");

	//Add event node 
	UGraphNodeDialogueChoice* TemplateObject = Cast<UGraphNodeDialogueChoice>(
		UGraphNodeDialogueChoice::MakeTemplate(ContextMenuBuilder.OwnerOfTemporaries,
			TargetGraph->GetAllSpeakers()[0]));
	check(TemplateObject);
	
	FText MenuText = LOCTEXT(
		"ChoiceNodeCreationText",
		"Choice"
	);
	FText MenuTooltip = LOCTEXT(
		"ChoiceNodeCreationTooltip",
		"Creates a node which serves as a player's choice."
	);
