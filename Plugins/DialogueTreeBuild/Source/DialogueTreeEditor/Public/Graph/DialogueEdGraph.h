...
public:
	...
	*/
	...
	/**
	* Checks if the graph has a speaker with the given tag. 
	* 
	* @param InTag - FGameplayTag, the tag of the speaker to check for. 
	* @return bool - True if the graph has the speaker. False otherwise. 
	*/
	bool HasSpeaker(FGameplayTag InTag) const;

	/**
	* Retrieves the color associated with the given speaker. White if no 
	* speaker is found. 
	* 
	* @param InTag - FGameplayTag of the target speaker. 
	* @return FColor - the color associated with the speaker in the graph. 
	*/
	FColor GetSpeakerColor(FGameplayTag InTag) const;
